...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'safetyAnalysisResult') {
    const { buttonId, score, reason, error } = message;
    
    const targetButton = document.querySelector(`.safety-check-btn[data-url="${CSS.escape(buttonId)}"]`);
    ...
    if (targetButton) {
      ...
      const wrapper = targetButton.parentNode;

      if (error) {
        targetButton.innerText = 'Error';
        targetButton.title = `Error: ${error}`;
        targetButton.style.color = '#5f6368';
        targetButton.style.borderColor = '#5f6368';
        targetButton.style.backgroundColor = '#f1f3f4';
      } else {
        targetButton.innerText = `${score}% Safe`;
